...
int main() {
    ...
    float input[k] = {1.0f, 2.0f, 3.0f, 4.0f}; 
    float output[m];
    ...
    CUDA_CALL(cudaMalloc(&d_input, k * sizeof(float)));
    CUDA_CALL(cudaMalloc(&d_output, m * sizeof(float)));
    ...
    CUDA_CALL(cudaMemcpy(d_input, input, k * sizeof(float), cudaMemcpyHostToDevice));
    ...
    CUDA_CALL(cudaMemcpy(output, d_output, m * sizeof(float), cudaMemcpyDeviceToHost));
    for (int i = 0; i < m; i++) {
