...
__device__ bool insert_hash_table(IdType* hash_table, IdType* round_id_table, IdType val, IdType round_id) {
  IdType pos = val % 50;
  IdType gap = 1;
  while (true) {
    if (round_id_table[pos] != round_id) {
      round_id_table[pos] = round_id;
      hash_table[pos] = val;
      return true;
    } else if (hash_table[pos] == val) {
      return false;
    } else {
      pos = (pos + gap) % 50;
      gap ++;
...
__global__ void sample_weighted_khop_hash_dedup(const IdType *indptr, const IdType *indices,
  ...
  IdType hash_table[100];
  for (int i = 0; i < 100; i++) {
    hash_table[i] = Constant::kEmptyKey;
  }
  IdType* round_id_table = hash_table + 50;
  ...
  for (size_t index = threadIdx.x + block_start; index < block_end;
    ...
    if (index < num_input) {
      ...
      if (len <= fanout) {
        ...
        size_t selected_unique = 0;
        for (; selected_unique < fanout;) {
          size_t k = curand(&local_state) % len;
          float r = curand_uniform(&local_state);
          IdType rand_result = indices[off + k];
          if (r > prob_table[off + k]) {
            rand_result = alias_table[off + k];
          }
          if (insert_hash_table(hash_table, round_id_table, rand_result, rid) == false) continue;
          tmp_src[index * fanout + selected_unique] = rid;
          tmp_dst[index * fanout + selected_unique] = rand_result;
          selected_unique++;
        }
      }
    }
...
void GPUSampleWeightedKHopHashDedup(const IdType *indptr, const IdType *indices,
  ...
  sample_weighted_khop_hash_dedup<Constant::kCudaBlockSize, Constant::kCudaTileSize>
      <<<grid, block, 0, cu_stream>>>(
          indptr, indices, prob_table, alias_table, input, num_input, fanout, tmp_src, tmp_dst,
