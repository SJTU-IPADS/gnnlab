...
void CUDA_CALL2(cudaError_t stat) {
   if (stat != cudaSuccess) {
 ...
 * [0, 1, 2, 0, 3, 4, 1, 2, 5, 3]
 *
 * \tparam uint32_t The type of the IDs.
 */
class DeviceOrderedHashTable {
  ...
  protected:
    ...
    static constexpr uint32_t kEmptyKey = static_cast<uint32_t>(0xffffffff);
    ...
    */
    inline __device__ uint32_t SearchForPosition(
        const uint32_t id) const {
      uint32_t pos = Hash(id);
