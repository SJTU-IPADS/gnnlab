...
*
* @tparam uint32_t The type of ID to store in the hashtable.
*/
class MutableDeviceOrderedHashTable : public DeviceOrderedHashTable {
  ...
  */
  explicit MutableDeviceOrderedHashTable(
      OrderedHashTable* const hostTable) :
...
*
* \tparam uint32_t The type of of id.
...
DeviceOrderedHashTable::DeviceOrderedHashTable(
...
OrderedHashTable::OrderedHashTable(
  ...
  CUDA_CALL2(cudaMalloc(&table_, sizeof(Mapping) * size_));

  CUDA_CALL2(cudaMemset(
    table_,
    DeviceOrderedHashTable::kEmptyKey,
    sizeof(Mapping)*size_));
}

OrderedHashTable::~OrderedHashTable() {
  CUDA_CALL2(cudaFree(table_));
}
...
void OrderedHashTable::FillWithDuplicates(
  ...
  generate_hashmap_duplicates<BLOCK_SIZE, TILE_SIZE><<<grid, block>>>(
      input,
      num_input,
      device_table);
  CUDA_CALL2(cudaGetLastError());


  uint32_t * item_prefix;
  CUDA_CALL2(cudaMalloc(&item_prefix, sizeof(uint32_t)*(num_input+1)));

  count_hashmap<BLOCK_SIZE, TILE_SIZE><<<grid, block>>>(
      input,
      num_input,
      device_table,
      item_prefix);
  ...
  CUDA_CALL2(cub::DeviceScan::ExclusiveSum(
      nullptr,
      workspace_bytes,
      ...
      grid.x+1));
  void * workspace;
  CUDA_CALL2(cudaMalloc(&workspace, workspace_bytes));
  ...
  CUDA_CALL2(cudaFree(workspace));

  compact_hashmap<BLOCK_SIZE, TILE_SIZE><<<grid, block>>>(
      input,
      num_input,
      device_table,
      item_prefix,
      ...
      num_unique);
  CUDA_CALL2(cudaGetLastError());
  CUDA_CALL2(cudaFree(item_prefix));
}

void OrderedHashTable::FillWithUnique(
    const uint32_t * const input,
    const size_t num_input) {
  ...
  generate_hashmap_unique<BLOCK_SIZE, TILE_SIZE><<<grid, block>>>(
      input,
      num_input,
      device_table);
