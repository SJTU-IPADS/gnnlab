import math, time
import sys, subprocess, numpy, os, re
...
def get_density_array(cfg_list : list, method='exp', n_slot=20, cache_percent=0):
  cmd = f"../common_solver/gen_density_matrix_fine_grain --coe {coefficient} -t {num_threads} -f " + " ".join([cfg.get_log_fname() + suffix for suffix in ["_optimal_cache_bin.bin", "_optimal_cache_freq_bin.bin"] for cfg in cfg_list]) + f" -m {method}" + f" -s {n_slot}" + f" -c {cache_percent}"
  print(cmd)
...
class Problem:
  ...
  def solve_cpp(self, cache_percent, T_local = 2, T_remote = 15, T_cpu = 60):
    t0 = time.time()
    cmd = f"../common_solver/solver_gurobi -f density.bin block_freq.bin -m {self.mode} -c {cache_percent} --tl {T_local} --tr {T_remote} --tc {T_cpu} -t {num_threads} --sm {' '.join([str(i) for i in self.stream_mapping])}"
    print(cmd)
