...
void constraint_connect_r_x(std::ostream & ss, uint32_t block_id, uint32_t part_id) {
  if (density_array[block_id] == 0) return;
  ss << "remote_req_1_" << block_id << "_" << part_id << ": ";
  ss << CPU_VAR(block_id) << " + " << GPU_VAR(block_id, part_id) << " <= 1\n";
}
...
int main(int argc, char** argv) {
  ...
  std::cout << "CPU VAR...\n";
  // FOR_LOOP(block_id, num_block) {
  //   if (ignore_block(block_id, density_array[block_id])) continue; ss << " " << CPU_VAR(block_id) << " <= 1" << "\n";
  // }
  do_parallel(ss, [](std::ostream & ss, uint32_t block_id){
    if (ignore_block(block_id, density_array[block_id])) return; ss << " " << CPU_VAR(block_id) << " <= 1" << "\n";
  }, 0, num_block);

  std::cout << "GPU VAR...\n";
  // FOR_LOOP(block_id, num_block) {
  //   if (ignore_block(block_id, density_array[block_id])) continue; FOR_LOOP(part_id, num_part) {ss << " " << GPU_VAR(block_id, part_id) << " <= 1" << "\n";}
  // }
  do_parallel(ss, [](std::ostream & ss, uint32_t block_id){
    if (ignore_block(block_id, density_array[block_id])) return; FOR_LOOP(part_id, num_part) {ss << " " << GPU_VAR(block_id, part_id) << " <= 1" << "\n";}
  }, 0, num_block);
  ss << "END\n";
  ss.close();
}