...
class Problem:
  ...
  def solve(self, cache_percent, T_local = 2, T_remote = 15, T_cpu = 60):
    ...
    solver = pulp.getSolver('GUROBI_CMD', options=[("MIPgap", 0.01), ("Threads", 54), ("Method", 2)])
    ...
    for block_id in range(self.num_block):
      for part_id in range(num_part):
        x_list[block_id][part_id] = pulp.LpVariable(f'x_block_{block_id:05d}_part_{part_id}', cat=pulp.LpBinary, e=None)
      c_list[block_id] = pulp.LpVariable(f'c_block_{block_id:05d}', cat=pulp.LpBinary, e=None)
    ...
    for block_id in range(self.num_block):
      ...
      try:
        ...
        for part_id in range(num_part):
          try_and_throw(round_x[part_id] + round_r[part_id] + round_c == 1)
...
for i in range(len(part1_conf_list.conf_list)):
  ...
  # for (cache_size, T_local, T_remote, T_cpu) in [(cache, 2, tr, tc) for tc in [60,100] for cache in range(11, 36, 2) for tr in range(2,16,2)]:
