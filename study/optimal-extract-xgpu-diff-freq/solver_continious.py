  ...
  def solve(self, cache_percent, T_local = 2, T_remote = 15, T_cpu = 60):
    ...
    for block_id in range(self.num_block):
      for part_id in range(num_part):
        x_list[block_id][part_id] = pulp.LpVariable(f'x_block_{block_id:05d}_part_{part_id}', lowBound=0,upBound=1, cat=pulp.LpContinuous, e=None)
      c_list[block_id] = pulp.LpVariable(f'c_block_{block_id:05d}', lowBound=0,upBound=1, cat=pulp.LpContinuous, e=None)
